...
export function Sidebar({
  ...
  return <div className="h-screen bg-sidebar flex flex-col border-r border-sidebar-border w-64">
      ...
      <div className="p-4 flex items-center justify-center h-16 border-b border-sidebar-border">
        ...
        <div className="text-treexpay-medium font-bold text-2xl">Nova Central</div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="space-y-1 px-2">
          ...
          return <li key={item.name}>
                <button onClick={() => handleNavigation(item.href)} className={cn("flex items-center w-full p-2 rounded-md transition-all duration-200 ease-in-out transform hover:scale-105", isActive ? "bg-sidebar-accent text-treexpay-medium" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground")}>
